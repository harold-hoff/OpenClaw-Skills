...
def load_credentials():
    ...
    # Support both skill-style (ALPACA_*) and official Alpaca env names (APCA_*).
...
def _nbbo_usable(bid: float, ask: float) -> bool:
    """True when bid/ask look like a real consolidated quote (free feeds often lie)."""
    ...
    if mid <= 0:
        return False
    # Very wide vs mid — e.g. AMC bid $1.39 ask $393 on IEX-derived NBBO.
    if spread / mid > 0.12:
        return False
    # Stale far-side: penny bid with huge ask multiple.
    if bid < 100 and ask >= bid * 5:
        return False
    return True


def _reference_price(bid: float, ask: float, last: Optional[float]) -> Tuple[Optional[float], str]:
    """
    Single price for sizing: prefer sane NBBO midpoint, else last sale.
    """
    if _nbbo_usable(bid, ask):
        return (bid + ask) / 2.0, "mid(BBO)"
...
def cmd_quote(args):
    """Latest NBBO plus a reference price (mid or last trade) when bid/ask is incomplete."""
    ...
    print(f"{'Symbol':<8} {'Bid':>12} {'Ask':>12} {'Spread':>10} {'Ref $':>10} {'Src':>8}")
    print("-" * 64)
    ...
    for symbol, quote in quotes.items():
        ...
        last = _num(tr.price) if tr is not None else None
        last_ok = last if (last is not None and last > 0) else None
        ref, src = _reference_price(bid, ask, last_ok)
        if _nbbo_usable(bid, ask):
            spread = ask - bid
            sp = f"${spread:>8.4f}"
        else:
            spread = float("nan")
            sp = f"{'—':>10}"
        rf = f"${ref:>8.2f}" if ref is not None else f"{'—':>10}"
        print(f"{symbol:<8} ${bid:>10.2f} ${ask:>10.2f} {sp:>10} {rf:>10} {src:>8}")

    print(
        "\nRef $ = midpoint only when NBBO looks sane; otherwise last sale. "
        "Free feeds often omit one side, zero out a side, or pair stale NBBO legs — use Ref $ for sizing."
    )
...
def cmd_bars(args):
    ...
    if symbol not in bars or not bars[symbol]:
        if getattr(args, "json", False):
            print(json.dumps({"symbol": symbol, "timeframe": args.timeframe, "bars": [], "error": "no_data"}))
        else:
    ...
    if getattr(args, "json", False):
        ...
        print(json.dumps({"symbol": symbol, "timeframe": args.timeframe, "bars": rows}))
        return
...
def main():
    ...
    account_parser.add_argument(
        "--json",
        action="store_true",
        help="Emit JSON for scripts (scan_report, automation)",
    )

    # Positions
    positions_parser = subparsers.add_parser("positions", help="List positions")
    ...
    bars_parser.add_argument(
        "--json",
        action="store_true",
        help="Emit JSON (bars array) for mean-reversion / automation",
    )
